...
6. If this machine connected to a memory repo that already has project memory in it, say that each project's memory has to be pointed at a folder here once: in the folder for that project, run `hearth project link` (and `hearth project show` to check). hearthkit says so at session start when a project is not linked yet, and it never guesses.
